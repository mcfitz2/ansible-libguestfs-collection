...
class guest:
    ...
    def bootstrap(self):
        ...
        if self.mounts and self.automount:
            results["msg"] = (
                "Automount (enabled by default) and manual "
        ...
        try:
            ...
            results["msg"] = (
                "Could not mount guest disk image, python exception: {}".format(str(e))
            )
            self.module.fail_json(**results)
        ...
        if self.automount:
            if len(roots) == 0:
                results["msg"] = (
                    "Automount failed, no devices were found in"
            ...
            for root in roots:
                ...
                if not filtered_mounts:
                    results["msg"] = (
                        "Failed to detect associated mountpoint for device {}.".format(
                            str(root)
                        )
                    )
                    self.module.fail_json(**results)
                try:
                    ...
                    results["msg"] = (
                        "Couldn't mount device inside guest disk"
                        "'image, python exception: {}".format(str(e))
                    )
                    self.module.fail_json(**results)
        else:
            if not self.mounts:
                results["msg"] = (
                    "Automount is disabled and no mountpoints were provided to module"
                )
                self.module.fail_json(**results)
            for mount_request in self.mounts:
                if len(mount_request.keys()) > 1:
                    results["msg"] = (
                        "Dictionary '{}' is expected to have a single key".format(
                            mount_request
                        )
                    )
                    self.module.fail_json(**results)
                ...
                try:
                    ...
                    results["msg"] = (
                        "Couldn't mount device inside guest "
                        "disk image, python exception: {}".format(str(e))
                    )
                    self.module.fail_json(**results)
        self.mount = True
        return self.handle
            ...
            if self.mount:
                ...
                if self.se_relabel:
                    ...
                    if re_policy:
                        ...
                        selinux_policy_line = list(
                            filter(re_policy.match, selinux_config)
                        )
                        if selinux_policy_line:
                            selinux_policy_string = re.search(
                                re_policy, selinux_policy_line[0]
                            )
                            selinux_policy = selinux_policy_string.group("policy")
                            selinux_spec_file = (
                                "/etc/selinux/{}/contexts/files/file_contexts".format(
                                    selinux_policy
                                )
                            )
                            if self.handle.exists(selinux_spec_file) == 1:
                                self.handle.selinux_relabel(
                                    selinux_spec_file, "/", force=True
                                )
                    else:
