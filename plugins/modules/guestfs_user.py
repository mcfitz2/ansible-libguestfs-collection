...
def users(guest, module):
    ...
    results = {"changed": False, "failed": False, "results": []}
    err = False
    ...
    if state == "present":
        if user_exists:
            ...
            if user_password:
                try:
                    guest.sh_lines(
                        "echo {u}:{p} | chpasswd".format(u=user_name, p=user_password)
                    )
                    changed = True
                    results["results"].append(
                        "Updated password for {}".format(user_name)
                    )
            ...
            if user_shell:
                try:
                    ...
                    changed = True
                    results["results"].append(
                        "Changed shell to {} for {}".format(user_shell, user_name)
                    )
                except Exception as e:
                    err = True
                    results["failed"] = True
            ...
            if user_groups:
                try:
                    ...
                    changed = True
                    results["results"].append(
                        "Added {} to groups: {}".format(user_name, groups_str)
                    )
                except Exception as e:
                    err = True
                    results["failed"] = True
            ...
            if not changed:
                results["results"].append(
                    "{} already exists with the specified configuration".format(
                        user_name
                    )
                )
            else:
            ...
            try:
                ...
                if user_groups:
                    results["results"].append(
                        "Added {} to groups: {}".format(
                            user_name, ",".join(user_groups)
                        )
                    )
                if user_shell:
                    results["results"].append(
                        "Set shell to {} for {}".format(user_shell, user_name)
                    )
...
def main():
    ...
    if (
        not module.params["password"]
        and module.params["state"] == "present"
        and not module.params.get("groups")
        and not module.params.get("shell")
    ):
        err = True
        results = {
