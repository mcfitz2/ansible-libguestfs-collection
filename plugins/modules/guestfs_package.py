...
PACKAGE_MANAGERS = {
    ...
    "apt": {
        "present": "apt-get -q -y -o Dpkg::Options::=--force-confnew update; apt-get -y install",
        "absent": "apt-get -y remove",
    },
}
...
def packages(guest, module):
    ...
    if module.params["name"]:
        ...
        if package_manager in PACKAGE_MANAGERS:
            try:
                result = guest.sh_lines(
                    "{command} {packages}".format(
                        command=PACKAGE_MANAGERS[package_manager][state],
                    ...
                    for package in module.params["name"]:
                        if package in line:
                            if "Verifying" in line:
                                ...
                                response.add(
                                    "{package} is {state}".format(
                                        package=invoked_package, state=state
                                    )
                                )
                            elif "already installed" in line:
                                response.add(line.replace("Package ", ""))
                            elif (
                                "No package {package} available.".format(
                                    package=package
                                )
                                in line
                            ):
                    ...
                    for package in module.params["name"]:
                        if package in line:
                            if "Unpacking" in line or "Removing" in line:
                                ...
                                invoked_package = re.sub(
                                    r"Unpacking|Removing", "", line
                                ).replace(" ...\r", "")
                                response.add(
                                    "{package} is {state}".format(
                                        package=invoked_package, state=state
            ...
            err = True
            results["msg"] = (
                "Package manager {package_manager} is not supported".format(
            ...
            if apps:
                for app in apps:
                    packages_list.append(
                        "{name}-{version}-{release}-{arch}".format(
                            name=app["app2_name"],
        ...
        if packages_list:
            ...
            err = True
            results["msg"] = (
                "Packages containing regular expression '{regexp}' not found".format(
                    regexp=app_regex
                )
            )
